...
const PhoneInput = ({ value, onChange, className }: { value: string; onChange: (value: string) => void; className?: string }) => {
                        ...
                        <CommandGroup>
                            {countries.map((country) => (
                                <CommandItem
                                    ...
                                    keywords={[country.name.normalize('NFD').replace(/[\u0300-\u036f]/g, ''), country.dialCode]}
                                    onSelect={(currentValue) => {
                                        onChange(currentValue);
                                        setOpen(false);
                                    }}
                                >
                                    <img loading="lazy" src={`/flags/${country.code}.svg`} alt={country.name} className="w-5 rounded-[3px] mr-2" />
                                    {country.name}
